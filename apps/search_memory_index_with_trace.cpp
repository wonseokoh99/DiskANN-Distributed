                        ...
                        const std::string& partition_file)
{
    ...
    if (trace_enabled) {
        std::ifstream p_fin(partition_file, std::ios::binary);
        if (!p_fin) {
            ...
            p_fin.close();
            std::cout << "Partition file loaded for tracing. " << partition_labels.size() << " labels." << std::endl;
        }
    }
    ...
    // *** 3. 추적 결과 저장용 벡터 선언 ***
    std::vector<std::vector<TraceInfo>> all_traces;
    if (trace_enabled) {
        all_traces.resize(query_num);
    }
...
int main(int argc, char **argv)
{
    std::string data_type, dist_fn, index_path_prefix, result_path, query_file, gt_file, filter_label, label_type,
        query_filters_file, partition_file; // partition_file 변수 추가
    ...
    try
    {
        ...
        optional_configs.add_options()("partition_file",
                                       po::value<std::string>(&partition_file)->default_value(""),
                                       "Path to the partition file to enable search path tracing.");
